...
本倉庫故意排除採集 `.bin`、`capture_*`、`auto_runs/`、分析圖片、日誌和本機狀態。例外僅為 `fpga/tang_primer_25k_hv7350/artifacts/` 及 `burn_test/` 下明確命名的FPGA燒錄位流，以及 `handoff/` 中不含採集資料的固件與SDK交付ZIP。克隆倉庫不包含任何採集資料；需要分析時把資料放入本機 `auto_runs/` 或在 UI 選擇外部資料夾。
