...
- Tang Primer 25K + HV7350 八通道發射波束形成FPGA工程及可回退位流。
...
16. [Tang Primer 25K + HV7350 FPGA基線](fpga/tang_primer_25k_hv7350/README.md)
...
docs/                          硬件和操作文檔
fpga/tang_primer_25k_hv7350/  2 MHz/2-cycle正單極HV7350發射基線
...
本倉庫故意排除採集 `.bin`、`capture_*`、`auto_runs/`、分析圖片、日誌和本機狀態。唯一例外是 `fpga/tang_primer_25k_hv7350/artifacts/` 下明確命名的FPGA燒錄位流。克隆倉庫不包含任何採集資料；需要分析時把資料放入本機 `auto_runs/` 或在 UI 選擇外部資料夾。
