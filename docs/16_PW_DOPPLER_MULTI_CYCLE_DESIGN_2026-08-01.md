...
| 參數 | 推薦值 | 理由 |
|---|---:|---|
| TX中心頻率 | 1.0 MHz已知5-level tapered pattern | 現行上位機預設；實際電氣與聲學中心頻率仍須用示波器/水聽器驗證 |
| PRF | 5 kHz | 25 mm深度裕量大，1.0 MHz、60°時Nyquist速度約3.85 m/s |
| TX burst | 先用已驗證2 cycles；SNR不足再驗證4 cycles | 周期數增加改善窄帶SNR，但不提高速度Nyquist |
| TX steering | 先0°定位；PW記錄時固定在單一角度，通常限制±4° | 1.59 mm pitch在1.0 MHz仍有柵瓣風險 |
...
以`c=1540 m/s`、`f0=1.0 MHz`計算：

| PRF | 無模糊深度 | 0°速度Nyquist | 60°速度Nyquist |
|---:|---:|---:|---:|
| 1 kHz | 770 mm | 0.385 m/s | 0.770 m/s |
| 2.5 kHz | 308 mm | 0.963 m/s | 1.925 m/s |
| 5 kHz | 154 mm | 1.925 m/s | 3.850 m/s |
| 10 kHz | 77 mm | 3.850 m/s | 7.700 m/s |
...
2. **TX電氣Gate**：假負載驗證1.0 MHz pattern的邊沿、周期、回零和T/R恢復。
