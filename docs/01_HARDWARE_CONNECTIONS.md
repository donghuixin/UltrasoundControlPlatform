...
J5到AFE建議用2×10轉16路SMA接口板和短50 Ω微同軸，不用杜邦線/麵包板。只使用8通道時，物理接A1–A8到AFE INP1–INP8；HSDC仍顯示16個數字槽是正常的，未接通道不等於有效陣元。
...
| 端口 | 方向/用途 | 當前成像 |
|---|---|---|
| TX J7 pin2 SYNCP/TR_BF_SYNC | 板載CPLD約2.5 V、約1 kHz輸出 | normal可留空；hardware可送TSW J13 |
...
SYNCP不是外部輸入口，不可把20 kHz函數發生器接回去。短距離單一TSW觸發可先用J7 pin2直連J13並共地，因為2.5 V低於3.3 V上限；長線、分配到多板或電平不確定時必須加3.3 V兼容緩衝/扇出並用示波器確認。
