...
Default: timing, buttons, UART integration, and the real 32-bit expiry boundary.
--full-duration additionally simulates all 250 million clocks of a 5 s session.
    ...
    with tempfile.TemporaryDirectory(prefix="probe-sequence-") as directory:
        for top in ("tb_probe_sequence", "tb_probe_duration"):
            ...
            if top == "tb_probe_duration" and args.full_duration and verilator:
                ...
                print("Running tb_probe_duration (compiled full five seconds)", flush=True)
            ...
            print(f"Running {top}" + (" (full five seconds)" if len(command) == 3 else ""),
                  flush=True)
