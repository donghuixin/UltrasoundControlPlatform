...
- 每次启动从第一枚 J11 上升沿计时 5 秒，然后自动停止，等待下一次按键。不会自动启动下一探头。
- 发射期间再次按 S2，立即中断旧通道并启动下一通道，重新计时 5 秒。按键仍有 20 ms 消抖；“立即”是有效按键被识别后，不等待旧的 5 秒结束。长按只产生一次事件。
- S1 无效。S2 不再是停止按钮。需要中途停止且不切换时，发送 USB `stop`；出现电气故障应断开高压供电，不依赖串口作为硬件急停。
- READY：发射期间亮，待机/结束时灭。DONE：只有自然满 5 秒后亮并保持；下一次启动或 USB STOP 清零。上电两灯灭。

一次完整发射窗口为 250,000,000 个 50 MHz 时钟，包含 **50,000 次 J11 触发和 50,000 组 burst**。手动中断的窗口不足 50,000 次。USB 拔掉不会延长窗口，硬件仍按时停止。
...
`start N` 直接启动指定探头 5 秒；即使该探头正在运行，也从头重新触发计时。`next` 与 S2 相同。`stop` 中断当前输出但保留下一探头游标。`status` 不改变发射。按键和 USB 共用游标，例如 `start 3` 之后下一次 S2/`next` 会启动 HV4。

START/NEXT/STOP 返回的是命令执行后的状态。每发一条普通命令，等待对应回复再发下一条；超时不能判断命令未执行，脚本不会自动重发 START。此时先查询状态或发送 STOP。没有自动“结束推送”，需要时轮询 STATUS；完成状态也可看 DONE。
...
回复共 9 字节：`5A A5 CMD SEQ RESULT ACTIVE NEXT FLAGS XOR`。最后一字节为 CMD 至 FLAGS 异或。RESULT：0 成功、1 未知命令、2 参数错误；ACTIVE：1～4，待机为 0；NEXT：1～4；FLAGS bit0=运行、bit1=正常结束，其余为 0。
...
5. 对提前切换的会话，不要假设收满 50,000 帧；丢弃被截断的末帧。若需要严格完整的四组数据，等每路自然满 5 秒、采集端完成保存后再启动下一路。
...
已生成可烧录文件：`burn_test/pmod_led_4probe_5s_2p2mhz_2cycles_10khz_uart.fs`。构建校验和见 [发布说明](../burn_test/README_four_probe_20260911.md)。
...
修改前源码完整保存在 `burn_test/source_before_four_probe_20260911.tar.gz`。旧八路自动发射 FS 保留为 `burn_test/pmod_led_FINAL_2p2mhz_2cycles_pos_fixed140ns_neg30v_100ns_autostart.fs`，未覆盖。

烧录新版本后先断开高压供电，确认上电无输出、S2 只选一路、5 秒停止，再恢复已排障的高压链路。烧录新 FS 与“更新代码”是两件事，本次工具不自动给实际硬件下载或发送 START。
