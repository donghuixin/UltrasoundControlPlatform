...
文档版本：1.1（持续发射）/ 2026-09-11。适用固件：`pmod_led_4probe_continuous_2p2mhz_2cycles_10khz_uart.fs`。协议帧格式不变，但取消旧版5秒自动停止；不能将两版发射时长行为混用。
...
4. `START(1..4)` 指定一路立即开始持续发射；`NEXT` 等同一次 S2；`STOP` 停止；`STATUS` 查询。
5. 上电不发射。一次只发射一路，直到按键/命令切换或 STOP；没有5秒自动停止，也不自动切到下一探头。READY运行亮，DONE恒灭。
6. 重复 START（即使同一路、同一 SEQ）也会重新产生 J11、重置 PRF 相位。**SEQ 只是回显，不提供去重或“恰好执行一次”保证。**
7. 一次只允许一个普通请求在途；等匹配回复后再发送下一条。不要让 GUI 定时器排队积累 STATUS。
8. 超时/断线/部分写入后状态未知，禁止自动重发 START/NEXT；先显式 STATUS 或 STOP 恢复判断。**关闭串口/拔USB后，只要板卡仍供电就会一直发射，不能再等5秒自动停机。**
...
| 文件/目录 | 用途 |
| --- | --- |
| `burn_test/pmod_led_4probe_continuous_2p2mhz_2cycles_10khz_uart.fs` | Gowin 可烧录位流 |
...
```text
a9d1b8992abd552fc892cd0dcef2ba1092a25f14236accc9eaa43549509c07e5
```

Windows PowerShell 校验：

```powershell
Get-FileHash .\burn_test\pmod_led_4probe_continuous_2p2mhz_2cycles_10khz_uart.fs -Algorithm SHA256
```

这是文件版本校验，不能通过 STATUS 判断实际板上正在运行哪个 FS。协议“v1”是线上帧格式约定，不是帧内字段；本文1.1是文档修订号，也不是设备版本回复。必须明确烧录本版，不要依靠运行中FLAGS=1区分旧5秒固件与本固件。旧八通道角度配置协议及 `configure_beamformer.py` 不适用于本版本。

持续发射交付ZIP为 `handoff/FPGA_4PROBE_CONTINUOUS_WINDOWS_HANDOFF_20260911.zip`。旧5秒FS及 `FPGA_4PROBE_WINDOWS_HANDOFF_20260911.zip` 保留回退。2026-09-15整理发布标签为 `fpga-hv7350-4probe-continuous-usb-20260915`，见[发布说明](RELEASE_CONTINUOUS_20260915.md)；固件构建日期及FS校验值不变，旧5秒标签不代表本版。
...
FLAGS 值使用十六进制，本版只有 bit0 可置1，bit1保留原 completed 的字段位置但恒为0，DONE恒灭。上位机不能再等待 completed=1 或 DONE 亮作为结束条件。上位机还应维护一个本地 **Unknown/未确认状态**：它不是 FPGA 返回值，而是连接刚建立或通信异常后的软件状态。Unknown 时显示“状态未确认”，不能显示“已停止”。

### 4.2 各操作的精确语义

| 操作 | 固件行为 |
| --- | --- |
| START N | 中止旧会话，选 N，重新产生 J11 并开始持续发射；NEXT=N%4+1 |
| START 当前 N | 也重新发 J11、重置 PRF 相位，绝不是查询或保持运行 |
| NEXT | 启动 NEXT 所指探头，随后游标加一并在 4 后回到 1 |
| S2 有效按下 | 与 NEXT 相同；首次 HV1，随后 HV2/HV3/HV4/HV1 |
| STOP | 中止输出；NEXT 不变；待机时也允许 |
| STATUS | 不主动改变发射，返回处理时的状态快照 |
| 无操作或经过5秒 | 原通道继续发射，不自动结束或换路 |
| 无效命令/参数 | 返回错误，不因该命令改变发射；原通道仍继续 |
...
本版本没有禁用 S2 的 USB 命令。按键可用于持续录制期间人工切探头；但仅靠稀疏 STATUS 轮询不能完整重建每次物理按键和精确帧边界。需要上位机精确编排时，应统一使用 USB 切换并约定不操作 S2。STATUS/无效请求本身不改变状态，不代表同时发生的按键不会改变状态。

### 4.3 持续时间和波形

从新会话第一枚**对外 J11 上升沿**开始，以10 kHz PRF持续发射；本版没有发射总时长或总帧数上限。每个PRF周期为5,000个50 MHz系统时钟，标称100 μs。经过5秒仍继续发射，不能把50,000帧当成硬件自动停止点。实际频率还受板上振荡器精度影响。
...
| CMD | 名称 | 合法 ARG | 上位机入口 |
| --- | --- | --- | --- |
| 10 | START | 01、02、03、04 | 启动指定探头持续发射 |
...
- 同一 START 或 NEXT 原样重发仍会执行，可能打断采集帧、重置PRF相位或切换探头。
...
| 7 | FLAGS | bit0=running；bit1保留原completed位置但本版恒0；bit2～7=0 |
| 8 | XOR | 偏移 2～7 六个字节异或；不包含帧头 |

有效回复必须同时满足：帧头和长度正确、XOR 正确、CMD/SEQ 匹配本次请求、RESULT 在 0～2、ACTIVE 在 0～4、NEXT 在 1～4、保留位为零、running 与 ACTIVE 非零一致。持续发射固件只产生FLAGS=00或01；客户端即使为兼容旧版保留completed属性，也不能据此等待本版自动结束。若收到completed=1，应核对实际烧录版本，不能当作本版正常结束。
...
本版没有自然结束事件，也没有主动 DONE 推送；物理 S2 也没有独立按键事件消息。STOP后ACTIVE=0、FLAGS=0，采集结束应由上位机的明确停止操作或记录策略决定，并结合接收端实际数据计数。STATUS仅为快照，不证明某个上位机会话ID已经结束或中间未被S2切换。
...
| 关闭应用/拔 USB | 不会停止 FPGA；只要板卡与高压链路仍供电就会一直发射，直到显式STOP或断电，不再有5秒期限 |
...
上面四条是独立操作示例，**不是应当一口气粘贴执行的采集脚本**；连续 START 会马上切换，每路不会自动结束。COM5 是占位，使用实际枚举值。CLI 每次独立开关端口，仅用于人工调试，不应作为主程序的并发控制架构；CLI运行结束不代表停止发射。

### 8.2 Python/PyQt 集成

使用 `host/python/fpga_probe_client.py`，按其 README 初始化一次客户端并复用。提供连接、指定探头启动、NEXT、STOP、STATUS 和关闭方法。示例 `example_integration.py` 的 `ContinuousAcquisition` 说明接收端只arm一次、持续读取数据、按需切换探头、最后显式停止的组织方式；其中 AFE/TSW API 只是适配位置，不是已经实现的采集驱动。
...
```text
上位机创建持续录制记录，配置 RX/ADC 通道和存储
    → 接收端 arm，确认已经能持续接收并等待 J11
    → 按 S2 启动 HV1，或 FPGA START N
    → 每枚 J11 硬件触发对应一次发射/采集
    → 接收端持续写盘、记录有效触发/帧数；可串行轮询 STATUS
    → 按 S2 或发送 NEXT/START 换探头，接收录制不重启
    → 丢弃切换边界受影响的数据，继续录制
    → 用户结束时，显式 FPGA STOP 并确认停机
    → 接收端 drain/停止录制并完成写盘，验证有效帧和质量
```

接收软件可以保持连续录制，探头驻留时间由按钮或上位机操作决定，不再绑定5秒或50,000帧。如果业务层以后需要按自定义时间自动切换，应由上位机显式发命令；FPGA本身不会自动跨探头扫描。本交付不改动已有接收UI或AFE/TSW驱动，也不保证现有接收设备已支持连续存储，需要接收侧自行确认带宽、缓冲和溢出检测。

STATUS不返回已发帧数，必须由接收硬件/采集程序实际统计。切换或STOP可能截断原J11、burst或J10，至少丢弃受影响的旧边界帧；新探头的首枚完整J11仍领先其发射2 μs。不能假定切换前后触发间隔恰好100 μs，也不能只按固定10 kHz时间网格强行补齐、归属探头。

协议没有每帧通道ID、按键事件时间戳或会话编号。人工S2切换时，低频STATUS只表示查询时正在发射哪一路，不能精准指出哪一帧发生切换，也可能漏掉查询间的多次切换。应保存观测到的状态与主机时间作为辅助记录，并排除无法确认归属的边界区间。即使USB控制切换，ACK到达时刻也不是首帧时刻；要求逐帧无歧义标签时，需要另行扩展硬件标记/协议和接收端，本版尚未提供。
...
所有输出共用寄存时序，通道切换有至少一拍 20 ns 全低间隔。切换会重置 PRF 相位，所以跨会话的 J11 间隔不保证 100 μs。J10是50 μs高电平的PRF参考，**不是持续录制使能，也不定义ADC必须只采50μs**。
...
| A04 | 一路不打断地运行超过5秒 | 继续10kHz输出，READY持续亮、DONE恒灭，无自动停止 |
| A05 | 运行中 NEXT / S2 | 旧路停止，新路持续发射，仍有完整2μs提前触发 |
| A06 | START同一路 | 当前会话重启、PRF相位重置，不是保持原波形相位 |
| A07 | HV4后NEXT | 回HV1；指定START 3后NEXT为HV4 |
| A08 | STOP后STATUS | ACTIVE=0、FLAGS=0，NEXT保留 |
| A09 | 运行超过5秒后STATUS | ACTIVE仍是当前探头、FLAGS=1，NEXT不变 |
| A10 | 错校验/不完整帧/错误停止位 | 不触发新操作，无正常ACK；原会话可继续 |
| A11 | 合法校验但START 5、未知CMD | 分别RESULT=2/1；原输出不因错误请求改变 |
| A12 | ACK丢失/关闭串口；仅低压逻辑验证 | 主机显示Unknown，不自动START重试；FPGA持续输出直到STOP或断电，无5秒兜底 |
| A13 | 先arm接收再START，保持录制并切换 | 首枚J11被采集；接收不中断，剔除切换边界数据，不声称STATUS提供逐帧标签 |
...
如以后需要这些能力，应新增明确协议命令和兼容性/版本判断，不能让上位机向保留字段写入新含义并假设当前FPGA支持。本交付的开发与验收范围是：**S2或Windows经USB串口选择HV1～HV4并启动/切换持续发射，USB STOP停止发射，同时为现有接收系统提供J11硬件同步。**

## 13. 黄金向量：上位机单元测试可直接使用

每行是独立用例，先建立相应初始状态，且处理时没有 S2 并发事件。`运行2→3` 表示 HV2 正在运行、NEXT=HV3。完整机器可读数据见 `tests/probe_protocol_vectors.json`。
...
| HV3持续运行、NEXT=4；STATUS | `A5 5A 13 00 07 14` | `5A A5 13 07 00 03 04 01 12` |
...
| HV3持续运行、NEXT=4；NEXT非法ARG=1 | `A5 5A 11 01 0A 1A` | `5A A5 11 0A 02 03 04 01 1F` |
| HV2运行；重放相同START/SEQ，重置PRF相位 | `A5 5A 10 02 02 10` | `5A A5 10 02 00 02 03 01 12` |

可在任意语言中验证：编码字节完全相同、分多次接收仍解码成功、校验错误不接受、SEQ不匹配不接受、两个flag矛盾不接受、超时不自动再发命令。以上不是让实物板连续执行的脚本，尤其重复 START 用例会中断旧帧、重置硬件PRF相位。

## 14. 本次验证范围

- 持续发射版已用Gowin V1.9.11.03 Education完成综合/布局布线并生成FS/BIN；50 MHz约束下Fmax=126.242 MHz，setup/hold违规端点均为0；本版FS的SHA-256见第2节。
- 持续发射版RTL仿真通过：Icarus覆盖上电、按键消抖/长按、四路切换、同路重启、精确波形及真实115200波特率协议；Verilator逐时钟运行完整6秒（300,000,000时钟），产生60,000帧、120,000个正向脉冲和60,000个负尾，第60,001帧仍输出，随后经UART STOP停止。DONE/FLAGS[1]始终为0，未force或跳过运行计时；独立UART错误输入/队列/全双工回归通过。
- Python客户端、持续录制示例和协议向量离线测试共53项通过；覆盖一次arm、多次切换不重启接收、显式STOP、结束录制前确认空闲及错误不自动重试，测试不启动实际发射。
